// Server-side XML parser for .flow.xml files.
// Uses @xmldom/xmldom (W3C DOM API) instead of the browser's DOMParser.
...
export function parseFlowXml(xml: string): ParsedFlow {
  ...
  // xmldom embeds <parsererror> on malformed input (same as browser).
  const parserError = doc.getElementsByTagName("parsererror")[0];
  if (parserError) {
    throw new FlowXmlParseError(
      `Malformed XML: ${parserError.textContent?.split("\n")[0] || "unknown"}`,
    );
  }
  ...
  const stopOnFailure =
    (textOf(root, "stopOnFailure") || "true").trim().toLowerCase() !== "false";
  ...
  if (stepEls.length === 0)
    throw new FlowXmlParseError("<steps> must contain at least one <step>");
...
function parseStep(el: XmlElement): ParsedStep {
  const numberAttr = el.getAttribute("number");
  const number = numberAttr ? parseInt(numberAttr, 10) : NaN;
  if (Number.isNaN(number))
    throw new FlowXmlParseError("<step> requires a numeric 'number' attribute");
  ...
  if (capturesRoot) {
    for (const cap of childElements(capturesRoot, "capture")) {
      ...
      if (!variable || !source) {
        throw new FlowXmlParseError(
          `Step ${number}: <capture> requires 'variable' and 'source' attributes`,
        );
      }
      const from: "response" | "request" | "computed" =
        fromAttr === "request" ? "request" : fromAttr === "computed" ? "computed" : "response";
    ...
    for (const a of childElements(assertionsRoot, "assertion")) {
      ...
      if (type === "status") {
        ...
        if (Number.isNaN(code))
          throw new FlowXmlParseError(`Step ${number}: status assertion missing 'code'`);
        assertions.push({ type: "status", code });
      } else if (type === "field-exists") {
        const field = a.getAttribute("field") || "";
        if (!field)
          throw new FlowXmlParseError(`Step ${number}: field-exists assertion missing 'field'`);
        assertions.push({ type: "field-exists", field });
      } else if (type === "field-equals") {
        const field = a.getAttribute("field") || "";
        const value = a.getAttribute("value") ?? "";
        if (!field)
          throw new FlowXmlParseError(`Step ${number}: field-equals assertion missing 'field'`);
        assertions.push({ type: "field-equals", field, value });
      } else if (type === "array-not-empty") {
        const field = a.getAttribute("field") || "";
        if (!field)
          throw new FlowXmlParseError(
            `Step ${number}: array-not-empty assertion missing 'field'`,
          );
...
function parseParams(root: XmlElement | null): Record<string, string> {
  if (!root) return {};
...
// xmldom's Element.children is not populated; iterate childNodes instead.
...
function childElements(parent: XmlElement, localName: string): XmlElement[] {
  ...
  for (let i = 0; i < nodes.length; i++) {
    const c = nodes[i];
    if (c.nodeType === 1 && (c as XmlElement).localName === localName) {
      out.push(c as XmlElement);
    }
  }
  return out;
}

function childElement(parent: XmlElement, localName: string): XmlElement | null {
  return childElements(parent, localName)[0] ?? null;
}

function textOf(parent: XmlElement, localName: string): string | undefined {
