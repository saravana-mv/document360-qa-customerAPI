...
3. **ENTITY DEPENDENCIES — CRITICAL**: Scan every endpoint's request body for foreign-key fields (any field ending in \`_id\` that references another resource, e.g. \`category_id\`, \`parent_id\`, \`folder_id\`, \`group_id\`). If a field description says "retrieve from GET /…" or the field name matches a sibling resource, the idea MUST include:
   ...
   **EXCEPTION — PROJECT VARIABLES REPLACE PREREQUISITES**: If a foreign-key field is already covered by an entry in "Available Project Variables" above, do NOT add a setup step to create that entity. Use \`{{proj.variableName}}\` directly in paths and body fields instead. For example, if \`{{proj.projectId}}\` is defined, omit the "Create Project" setup step and reference the variable wherever \`{project_id}\` appears. Only generate prerequisites for entities that have no matching project variable.
