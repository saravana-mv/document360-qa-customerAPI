...
1. **STRICT SCOPE**: Only use API endpoints, methods, and paths explicitly described in the provided spec files. Do not invent endpoints. **Exception**: prerequisite setup/teardown steps for entity dependencies (rule 2) are allowed — but you MUST use the same API version prefix (e.g., \`/v3/\`) as the provided specs. NEVER use a different version.
