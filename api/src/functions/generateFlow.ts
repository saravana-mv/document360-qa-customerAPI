...
**CRITICAL**: Use the EXACT field names from the spec schema — copy field names character-for-character from the schema properties list. Do NOT substitute similar-sounding names (e.g., different endpoints may use \`title\` or \`name\` — always check THAT endpoint's spec). For prerequisite steps without a spec, use the most common field name for that resource type.
...
11. **Request body fields MUST match the SPECIFIC endpoint's schema**: Only include fields that are defined in the endpoint's request body schema. Do NOT add extra fields like \`project_version_id\` unless the spec explicitly lists them in the request body for that specific endpoint. Many APIs use \`additionalProperties: false\` and will reject or error on unknown fields. Also never send fields marked \`readOnly: true\` in request bodies — those are response-only fields. **CRITICAL for PATCH/PUT update steps**: The update endpoint's request schema is often DIFFERENT from the create endpoint's schema. Fields like \`workspace_id\`, \`project_version_id\`, or \`category_type\` may be valid on POST (create) but NOT accepted on PATCH (update). Always check the UPDATE endpoint's own request body schema — do NOT copy fields from the create step's body into the update step.
