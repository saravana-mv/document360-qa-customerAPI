...
13. **Per-Step Context**: When a "# Per-Step Context" section is provided in the user message, follow it precisely — it maps each step to its spec file, lists the required body fields with recommended values, and specifies which response fields to capture and pass to later steps. Use the exact field names, value sources, and capture variables listed there. Do NOT deviate from the field names or value sources unless the spec reference section shows different information. **IMPORTANT**: Rules 2 (entity dependencies) and 4 (mandatory teardown) still apply — if the Per-Step Context is missing a prerequisite or teardown step that is clearly needed, ADD it.
