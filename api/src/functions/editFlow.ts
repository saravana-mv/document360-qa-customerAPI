...
10. **Request body MUST include ALL required fields with EXACT names**: When the spec or API rules document a request body schema, every field listed as \`required\` must be present in the \`<body>\` CDATA using the EXACT field name from the schema. Do NOT substitute similar names (e.g., do NOT use \`name\` when the spec says \`title\`). If the existing XML has wrong field names, rename them to match the spec. If required fields are missing, add them. Use project variables (\`{{proj.X}}\`), state variables (\`{{state.X}}\`), or sensible test values.
