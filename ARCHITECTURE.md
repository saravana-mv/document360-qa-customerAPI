...
├── common/         # Layout, TopBar, SideNav, Modal, ContextMenu, XmlCodeBlock, XmlEditor, ResizeHandle, ProjectPicker, QualityScorePill (colored 0-100% pill — red <50 / amber 50-79 / green ≥80, native title tooltip; used by FileTree)
├── specfiles/      # FileTree (with _system and _distilled folder support: isSystem flag, lock icon, read-only; renders QualityScorePill per file/folder for non-system nodes), FlowChatPanel, FlowIdeasPanel, FlowsPanel, DetailPanel, ImportFromUrlModal, ImportResultModal (post-import stats + variable/connection auto-detect + processing health banner), ReimportSpecModal (destructive reimport with validation-before-wipe, type-to-confirm), FolderRulesPanel, NewVersionModal, QualityScoreBanner (collapsible factor breakdown grouped by failing/partial/passing/skipped, used by EndpointDocView)
├── apidocs/        # TryItPanel (live request runner inside Spec Manager — passes specPath + versionFolder; renders "Enhance Docs example" button next to Send when a response exists, qa_manager+ only), EnhanceDocsExampleModal (loading/review/error states, XmlDiffView, cost display, summary chips, Confirm & save → calls existing uploadSpecFile, distillation auto-fires), EndpointDocView (renders QualityScoreBanner between method/path box and operation description)
...
|--------|---------------|
| `specFilesApi.ts` | `listSpecFiles`, `importSpecFileFromUrl`, `syncSpecFiles`, `getSourcesManifest`, `updateSourceUrl`, `generateFlowIdeas`, `splitSwagger`, `reimportSpec`, `deleteSpecFolder` (server-side bulk delete via `?prefix=X`) — `SplitSwaggerResult` includes `suggestedVariables` (`SuggestedVariable[]`), `suggestedConnections` (`SuggestedConnection[]`), and optional `processing` (`ProcessingReport`: distillation totals/errors + digest build status). Uses raw `fetch()` (not `apiClient`), so includes its own 401→`session-expired` event dispatch. |
| `flowApi.ts` | `generateFlowXml` (AI generation from plan — accepts optional `ideaId`, `versionFolder`, `folderPath` for server-side spec selection), `editFlowXml` (accepts optional `method` and `path` for spec-aware editing) |
...
| `enhanceDocsExampleApi.ts` | `enhanceDocsExample()` POSTs to `/api/spec-files/enhance-example` with the captured Try-it request/response and spec MD path. Returns `{ originalMd, updatedMd, costUsd, summary }` for the diff modal. Surfaces typed errors via the `EnhanceDocsExampleError` class (codes: `redaction_incomplete`, `ai_invalid_json`, `credit_denied`, `forbidden`, etc.) so the UI can render specific guidance per failure mode. |
...
|----------|-------|---------|---------|
| `specFiles` | `/api/spec-files` | GET/POST/PUT/DELETE | Spec file CRUD + content download. DELETE supports `?name=X` (single file) and `?prefix=X` (server-side bulk delete via `batchDeleteByPrefix`). Listing remaps `_distilled/` blobs into `_system/_distilled/` virtual paths; read requests reverse-map back to actual blob locations. Returns 404 (not 500) for missing blobs. |
...
| `generateFlow` | `/api/generate-flow` | POST | AI XML generation from confirmed plan. Accepts `ideaId` + `versionFolder` + `folderPath` for server-side spec resolution (looks up idea from Cosmos, lists/filters blob specs, reads distilled content). Falls back to client-provided `specFiles` for ad-hoc prompts. |
...
| `projects` | `/api/projects` | GET/POST/PUT/DELETE | Project CRUD (GET filtered by membership, POST project_owner+, PUT/DELETE project_owner+). DELETE uses `batchDeleteByPrefix()` for spec blob cleanup. |
...
| `specFilesSplitSwagger` | `/api/spec-files/split-swagger` | POST | Split OpenAPI 3.x / Swagger 2.x JSON spec into per-endpoint .md files by tag. Stores original spec at `_system/_swagger.json` (with legacy fallback read). Resolves $refs, creates tag-based folders, uploads through `distillAndStore` pipeline. Awaits `batchDistillAll` to collect per-file results, then eagerly calls `rebuildDigest()`. Returns `suggestedVariables` (path parameters), `suggestedConnections` (security schemes), and `processing` report (`{ distillation: { total, distilled, unchanged, errors, errorDetails }, digest: { built, error? } }`). Exports `batchUpload`, `batchDistillAll`, `DistillResult` for reuse by reimport. Batch helpers (`batchUpload`, `batchDelete`) extracted to `specBatchHelpers.ts`. |
| `specFilesReimport` | `/api/spec-files/reimport` | POST | **Reimport OpenAPI spec** into existing version folder. Validates spec before wiping anything. Preserves `_skills.md` and `_rules.json`. Wipes spec blobs via `batchDelete()`, ideas, flows, active tests (folder-scoped), and flow chat sessions. Then runs full split-swagger pipeline (upload, distill, digest, dependencies). Returns `suggestedVariables`, `suggestedConnections`, and `processing` report. Audit logged as `spec.reimport`. |
| `enhanceDocsExample` | `/api/spec-files/enhance-example` | POST | **AI-rewrite spec MD example block** from a captured Try-it request/response. Role-gated `qa_manager+`. Surgically extracts the OpenAPI JSON block from the spec MD (preserving the markdown wrapper byte-equal via `extractOpenApiBlock`/`spliceOpenApiBlock`), locates the operation via path-template-vs-captured-URL match (`findOperation`), slices only the request/response subtrees for the AI (`extractTargetSlice`/`applyTargetSlice`). Pre-strips auth headers (`stripAuthHeaders`) before AI sees data. AI replaces IDs in `*_id`/`*Id`/`id`/`*_uuid`/`*_key`/`*_token` fields and URL path params with `{{proj.<name>}}` placeholders; redacts JWTs, Bearer tokens, emails, phone numbers, internal hostnames; PRESERVES slugs, titles, status enums, non-ID-keyed values. Server post-pass (`detectResidualSecrets`) scans AI output for residual secrets — returns **422 redaction_incomplete** if any are found. 64KB cap on captured bodies (`truncateForAi`). One retry on malformed JSON, then **422 ai_invalid_json**. **402** on credit denial. `maxTokens: 4096`. Endpoint does NOT persist — returns `originalMd` + `updatedMd` for diff modal review. Audit action: `spec.enhance_example` (fired when AI runs, independent of eventual `spec.update` on save). |
...
| `blobClient.ts` | Azure Blob Storage (upload, download, list, delete, exists, `batchDeleteByPrefix()` — lists blobs under prefix and deletes in batches of 50 with `Promise.allSettled`) |
...
| `auditLog.ts` | Fire-and-forget `audit()` function, writes to Cosmos `audit-log` container. Actions include `project.member_add`, `project.member_remove`, `project.member_role_change`, `project.variables.update`, `project.apiRules.update`, `spec.bulk_delete`, `spec.reimport`, `spec.enhance_example`. |
...
| `specBatchHelpers.ts` | Batch blob operations: `batchUpload()` and `batchDelete()` — shared by split-swagger, reimport, and project delete |
| `specFileSelection.ts` | Server-side spec file selection for flow generation. `filterRelevantSpecs(specFiles, idea)` scores spec files against idea endpoints/title and returns the most relevant matches. Used by `generateFlow` to resolve specs from `ideaId` without client-side file lists. |
...
| `specDigest.ts` | Scalable spec context for large projects. Three tiers: Raw → Distilled → Digest (~2-3 lines/endpoint). Builds `_system/_digest.md` blob per version folder (lightweight endpoint index grouped by resource). Filters out `_system/` files from digest builds. `rebuildDigest()` reads blobs in parallel batches of 30 with `Promise.allSettled`. Threshold: >20 specs use digest. Auto-invalidated on spec file changes. **Eagerly rebuilt during OpenAPI import** (split-swagger awaits distillation then calls `rebuildDigest`); lazy rebuild in `generateFlowIdeas` remains as fallback. |
| `swaggerSplitter.ts` | Core OpenAPI/Swagger splitting logic: recursive $ref resolution (with circular detection), tag-to-folder kebab-case naming, method-to-filename with collision handling, Swagger 2.x→3.x normalization, per-endpoint markdown builder. `extractPathParameters()` extracts path params from all endpoints as `SuggestedVariable[]` with `toCamelCase()` normalization and deduplication of case variants (e.g., `projectId`/`ProjectId` → `projectId`), merged folder lists, and `simplifyType()` for clean type display. `extractSecuritySchemes()` extracts OAuth2/Bearer/API Key/Basic/Cookie auth from security definitions as `SuggestedConnection[]`. |
| `specOpenApiBlock.ts` | Pure helpers for the **Enhance Docs example** flow. `extractOpenApiBlock(md)` finds the OpenAPI JSON code block in a spec MD and returns `{ json, start, end }` preserving exact byte offsets; `spliceOpenApiBlock(md, region, newJson)` splices replacement JSON back into the same byte range so the markdown wrapper stays byte-equal. `findOperation(spec, method, capturedUrl)` matches a path template vs a captured URL (handles `{param}` substitution). `extractTargetSlice(operation)` carves out only the request/response subtrees to send to the AI; `applyTargetSlice(operation, slice)` merges the AI-rewritten slice back into the operation in place. |
| `exampleSanitization.ts` | Pure helpers for the **Enhance Docs example** flow. `stripAuthHeaders(headers)` removes `Authorization`, `Cookie`, and secret-suggesting headers before any data reaches the AI. `detectResidualSecrets(text, context)` scans AI output for JWTs, `Bearer` prefixes, and the captured Authorization value — returns the list of detected leaks for the server post-pass (triggers 422 if non-empty). `truncateForAi(body, cap = 64KB)` enforces the 64KB cap on captured request/response bodies. |
...
### Spec Quality Score (Client-Side)
`src/lib/spec/specQuality.ts` is a pure scoring function (no I/O, no persistence) that rates each parsed OpenAPI spec MD 0-100% on factors that affect AI flow generation quality. 19 factors across 6 groups (operation metadata, parameters, request body, responses, schema depth, security) each contribute weighted points; skipped factors are excluded from the denominator (e.g., security scoring is skipped on a public endpoint). Public API: `computeEndpointScore(endpoint)`, `computeSpecQuality(spec, versionFolder)`, `bandFor(score)`. Exported types: `EndpointScore`, `FolderScore`, `FactorResult`, `FactorStatus`, `SpecQuality`. Computed once per `parsedSpec` change in `SpecFilesPage` via `useMemo`; results passed down via the `qualityScores?: SpecQuality` prop into `<FileTree/>` (renders `<QualityScorePill/>` per node, skipping `isSystem` nodes since `_system`/`_distilled` are not in `buildEndpointFileMap`) and via `qualityScore?: EndpointScore` into `<EndpointDocView/>` (renders `<QualityScoreBanner/>`). Bands: red <50, amber 50-79, green ≥80. Server-side caching is deferred — the same pure function lifts into an Azure Function if/when the project-tile portfolio view becomes a priority.
