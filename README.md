...
- **Spec Quality Score** — Each spec MD is rated 0-100% on the factors that affect AI flow generation quality (descriptions, examples, error responses, schema depth, security, parameter completeness — 19 factors across 6 groups). A colored pill (red <50 / amber 50-79 / green ≥80) appears next to every file and folder (folder = aggregate) in the Spec Manager file tree, and the Documentation tab shows a collapsible banner with the full factor breakdown grouped by failing/partial/passing/skipped. Scores are computed client-side on the fly and update automatically when "Enhance Docs example" saves a spec.
...
- Project-level variables (`{{proj.varName}}`) — shared key/value pairs interpolated at runtime in both browser and server runners; also used to resolve `{any_name}` path parameters in flow XML (e.g., `{project_id}` maps to `{{proj.project_id}}`). All variable references in flow XML use `{{…}}` mustache syntax uniformly.
