...
import { buildTree, type TreeNode, type FolderNode } from "../components/specfiles/FileTree";
...
export function IdeasFlowsPage() {
  ...
  // ── File tree state (for folder nav) ──────────────────────────────────────
  ...
  // ── Load file list (for folder tree) ──────────────────────────────────────
  ...
  // Rehydrate selection on first file load
  const didRehydrateRef = useRef(false);
  useEffect(() => {
    if (didRehydrateRef.current || loadingFiles || files.length === 0) return;
  ...
  const folderMdCount = activePath
    ? (() => {
  ...
  const noSpecFiles = !projectHasSpecFiles || folderMdCount === 0;
  ...
  // Build a filtered tree — only show version folders and their non-system children
  const folderTree = useMemo(() => {
    const fullTree = buildTree(files);
    // Filter: keep only version root folders + their non-system resource subfolders
    function filterNode(node: TreeNode): TreeNode | null {
      if (node.isSystem) return null;
      if (node.type === "file") return null; // No individual files in folder nav
      // Keep folder, recurse into children
      const filteredChildren = node.children
        .map(filterNode)
        .filter((n): n is TreeNode => n !== null);
      return { ...node, children: filteredChildren } as FolderNode;
    }
    return fullTree
      .map(filterNode)
      .filter((n): n is TreeNode => n !== null);
      ...
      <div className="h-full flex overflow-hidden">
        ...
        <aside className="shrink-0 bg-white flex flex-col overflow-hidden border-r border-[#d1d9e0]" style={{ width: treeWidth }}>
          <div className="flex items-center gap-2 px-3 h-10 border-b border-[#d1d9e0] bg-[#f6f8fa] shrink-0">
            ...
            <span className="text-sm font-semibold text-[#1f2328]">Folders</span>
          </div>
          <div className="flex-1 overflow-y-auto py-1">
            {loadingFiles ? (
              ...
              <div className="px-3 py-6 text-center text-sm text-[#656d76]">
                No spec folders yet. Import an API spec in the Specs page first.
              </div>
            ) : (
              <FolderNavTree
                nodes={folderTree}
                ...
                <div className="flex-1 flex items-center justify-center bg-white">
                  <div className="text-center space-y-4 max-w-sm">
                    <div className={`w-14 h-14 rounded-full flex items-center justify-center mx-auto ${
                      folderMdCount === 0 ? "bg-[#656d76]/10" : "bg-[#0969da]/10"
                    }`}>
                      <svg className={`w-7 h-7 ${folderMdCount === 0 ? "text-[#656d76]" : "text-[#0969da]"}`} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-[#656d76]">
                        {noSpecFiles
                          ? (!projectHasSpecFiles
                              ? "Upload spec files (.md) in the Specs page to get started with AI-powered test flow generation."
...
// ── FolderNavTree — simple collapsible folder tree for navigation ──────────

function FolderNavTree({ nodes, selectedPath, pathsWithIdeas, onSelectFolder, depth = 0 }: {
  nodes: TreeNode[];
  ...
  const [expanded, setExpanded] = useState<Set<string>>(() => {
    ...
    return new Set(nodes.map(n => n.path));
  });
  ...
  return (
    <>
      {nodes.map(node => {
        ...
        const hasIdeas = pathsWithIdeas.has(node.path);
        const hasChildren = node.children.some(c => c.type === "folder");
        // Check if any descendant has ideas
