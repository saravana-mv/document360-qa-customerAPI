...
export function SpecFilesPage() {
  ...
  function buildSkillsTemplate(folderPath: string, variables?: SuggestedVariable[]): string {
    ...
    return [
      ...
      "## API Rules",
      "",
      "- When a test flow targets an endpoint that requires a dependent resource",
      "  (e.g. creating an article needs a category), the flow MUST create all",
      "  prerequisites as setup steps and delete them in reverse order as teardown",
      "  steps. **Exception**: entities listed in 'Context Variables' below are",
      "  pre-provisioned — use their `{{proj.*}}` variables directly and do NOT",
      "  add setup or teardown steps for them. Prerequisite endpoints may belong",
      "  to a different resource folder (e.g. the categories spec may be in a",
      "  sibling folder) — always include them in the flow even when generating",
      "  from a single folder's specs.",
      "",
