...
export async function patchArticle(projectId: string, articleId: string, body: Record<string, unknown>, token: string, langCode = "en"): Promise<Article> {
  const resp = await apiClient.patch<{ data: Article }>(`/v3/projects/${projectId}/articles/${articleId}?lang_code=${langCode}`, body, token);
  return resp.data;
}
...
export async function getArticleSettings(projectId: string, articleId: string, token: string, langCode = "en"): Promise<unknown> {
  const resp = await apiClient.get<{ data: unknown }>(`/v3/projects/${projectId}/articles/${articleId}/settings?lang_code=${langCode}`, token);
  return resp.data;
}
...
export async function patchArticleSettings(projectId: string, articleId: string, body: unknown, token: string, langCode = "en"): Promise<unknown> {
  const resp = await apiClient.patch<{ data: unknown }>(`/v3/projects/${projectId}/articles/${articleId}/settings?lang_code=${langCode}`, body, token);
  return resp.data;
}

export async function getWorkflowStatuses(projectId: string, token: string): Promise<unknown[]> {
  const resp = await apiClient.get<{ data: unknown[] }>(`/v3/projects/${projectId}/workflow-statuses`, token);
  return resp.data || [];
}
