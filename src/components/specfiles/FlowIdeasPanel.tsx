  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[#d1d9e0] bg-[#f6f8fa] shrink-0">
        <svg className="w-4 h-4 text-[#8250df]" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 18v-5.25m0 0a6.01 6.01 0 0 0 1.5-.189m-1.5.189a6.01 6.01 0 0 1-1.5-.189m3.75 7.478a12.06 12.06 0 0 1-4.5 0m3.75 2.383a14.406 14.406 0 0 1-3 0M14.25 18v-.192c0-.983.658-1.823 1.508-2.316a7.5 7.5 0 1 0-7.517 0c.85.493 1.509 1.333 1.509 2.316V18" />
        </svg>
        <span className="text-xs font-semibold text-[#1f2328]">Ideas</span>
        {totalIdeas > 0 && (
          <span className="text-[10px] px-1.5 py-px rounded-full font-medium bg-[#8250df]/10 text-[#8250df] border border-[#8250df]/20">{totalIdeas}</span>
        )}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-12 gap-2">
            <svg className="w-5 h-5 text-[#8250df] animate-spin" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
            </svg>
            <p className="text-xs text-[#656d76]">Generating ideas...</p>
          </div>
        )}
      ...
      {!loading && ideas && ideas.length > 0 && (
        <div className="shrink-0 border-t border-[#d1d9e0] bg-[#f6f8fa] px-3 py-2 flex gap-2">
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1.5 bg-[#1a7f37] hover:bg-[#1a7f37]/90 disabled:bg-[#eef1f6] disabled:text-[#656d76] disabled:border-[#d1d9e0] text-white text-xs font-medium rounded-md px-3 py-1.5 transition-colors border border-[#1a7f37]/80 disabled:border-[#d1d9e0]"
          >
