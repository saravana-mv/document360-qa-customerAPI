...
import type { ValidationResult } from "../../lib/api/validateFlowApi";
...
interface Props {
  ...
  folderPath?: string;
  /** Spec validation results (from "Validate against spec" action) */
  specValidation?: ValidationResult | null;
  /** Clear the spec validation results */
  onClearSpecValidation?: () => void;
}

type FlowTab = "idea" | "flow-xml" | "validation";
...
function ValidationContent({ result, onClear }: { result: ValidationResult; onClear?: () => void }) {
  const { summary, issues } = result;

  // Group issues by step number (null = flow-level)
  const grouped = useMemo(() => {
    const map = new Map<number | null, typeof issues>();
  ...
  const sortedKeys = [...grouped.keys()].sort((a, b) => (a ?? -1) - (b ?? -1));

  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      {/* Summary bar */}
      <div className={`shrink-0 px-4 py-2.5 border-b border-[#d1d9e0] flex items-center gap-3 ${
        summary.errors > 0
          ? "bg-[#ffebe9]"
          : summary.warnings > 0
            ? "bg-[#fff8c5]"
        ...
        <div className="flex items-center gap-2 flex-1">
          {summary.errors > 0 ? (
            ...
            <svg className="w-5 h-5 text-[#9a6700]" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9 3.75h.008v.008H12v-.008Z" />
            </svg>
          ) : (
            <svg className="w-5 h-5 text-[#1a7f37]" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
          ...
          {summary.info > 0 && (
            <span className="text-xs text-[#656d76]">{summary.info} info</span>
          )}
        </div>
        {onClear && (
          <button
            onClick={onClear}
            className="text-xs text-[#656d76] hover:text-[#1f2328] px-2 py-0.5 rounded-md hover:bg-black/5 transition-colors"
          >
            Dismiss
          </button>
        )}
      ...
      <div className="flex-1 overflow-y-auto">
        {issues.length === 0 ? (
          <div className="text-center py-8 text-sm text-[#656d76]">No issues found</div>
        ) : (
                    ...
                    {stepIssues.map((issue, i) => (
                      <div key={i} className="flex items-start gap-2">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {issue.suggestion && (
                            <div className="text-xs text-[#656d76] mt-0.5">{issue.suggestion}</div>
                          )}
...
export function DetailPanel({ selectedIdea, selectedFlow, flowIdea, onDownloadFlow, onGenerateFlow, generatingFlows, isFlowMarked, onCreateTest, creatingTest, onUpdateFlowXml, isFlowLocked, flowLockTooltip, canUnlockFlow, onUnlockFlow, folderPath, specValidation, onClearSpecValidation }: Props) {
  ...
  // Auto-switch to validation tab when spec validation results arrive
  const prevValidationRef = useRef<ValidationResult | null | undefined>(undefined);
  if (specValidation && specValidation !== prevValidationRef.current) {
    prevValidationRef.current = specValidation;
    setActiveTab("validation");
  } else if (!specValidation && prevValidationRef.current) {
      ...
      <div className="flex flex-col h-full overflow-hidden">
        ...
        {hasTabs && (
          <div className="flex items-center gap-1 px-4 h-9 border-b border-[#d1d9e0] bg-[#f6f8fa] shrink-0">
            {(["idea", "flow-xml", ...(specValidation ? ["validation" as FlowTab] : [])] as FlowTab[]).map((tab) => (
              <button
                key={tab}
              ...
              >
                {tab === "idea" ? (
                  ...
                  <span className="inline-flex items-center gap-1">
                    ...
                    Validation
                    {specValidation && specValidation.summary.errors > 0 && (
                      <span className="text-[10px] px-1 py-px rounded-full bg-[#d1242f] text-white font-medium">{specValidation.summary.errors}</span>
                    )}
                    {specValidation && specValidation.summary.errors === 0 && specValidation.summary.warnings > 0 && (
                      <span className="text-[10px] px-1 py-px rounded-full bg-[#9a6700] text-white font-medium">{specValidation.summary.warnings}</span>
                    )}
                  </span>
                )}
              </button>
        ...
        {/* Tab content */}
        {hasTabs && activeTab === "validation" && specValidation ? (
          <ValidationContent result={specValidation} onClear={onClearSpecValidation} />
