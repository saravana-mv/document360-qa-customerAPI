  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 h-10 border-b border-[#d1d9e0] bg-[#f6f8fa] shrink-0">
        ...
        <button
          onClick={() => { void navigator.clipboard.writeText(content).then(() => { setCopied(true); setTimeout(() => setCopied(false), 1500); }); }}
          title={copied ? "Copied!" : "Copy markdown"}
          className={`shrink-0 w-7 h-7 flex items-center justify-center rounded-md transition-colors border ${copied ? "text-[#1a7f37] bg-[#dafbe1] border-[#aceebb]" : "text-[#656d76] hover:text-[#1f2328] hover:bg-[#eef1f6] border-transparent hover:border-[#d1d9e0]"}`}
        >
          {copied ? (
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
