      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 flex flex-col" style={{ maxHeight: "80vh" }}>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200">
          ...
          <button
            onClick={onClose}
            disabled={uploading}
            className="text-gray-400 hover:text-gray-600 disabled:opacity-40 rounded p-1"
          >
        ...
        <div
          ...
          className={`mx-4 mt-4 flex flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed py-8 cursor-pointer transition-colors ${
        ...
        >
          ...
          <p className="text-xs text-gray-400">.md · .xml · .xsd · .txt</p>
          <input
            ref={inputRef}
            type="file"
            multiple
        ...
        {items.length > 0 && (
          <div className="flex-1 overflow-y-auto mx-4 mt-3 space-y-1">
            {items.map((item) => (
              <div key={item.file.name} className="flex items-center gap-2 text-xs rounded px-2 py-1.5 bg-gray-50">
                {item.status === "pending" && <span className="w-4 h-4 rounded-full border-2 border-gray-300 shrink-0" />}
                ...
                <span className="flex-1 truncate font-mono text-gray-700">{item.file.name}</span>
                {item.status === "error" && (
                  <span className="text-red-500 truncate max-w-24" title={item.error}>{item.error}</span>
                )}
                {item.status === "pending" && (
                  <button
                    onClick={() => removeItem(item.file.name)}
                    className="text-gray-400 hover:text-red-500 rounded p-0.5 shrink-0"
                  >
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-2 px-4 py-3 border-t border-gray-200 mt-3 shrink-0">
          <button
            onClick={onClose}
            disabled={uploading}
            className="text-xs text-gray-500 hover:text-gray-800 border border-gray-200 rounded px-3 py-1.5 hover:bg-gray-50 disabled:opacity-40"
          >
          ...
          {!allDone && (
            <button
              ...
              className="text-xs font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:bg-gray-200 disabled:text-gray-400 rounded px-3 py-1.5 transition-colors"
            >
