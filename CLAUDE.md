...
25+ Azure Functions. All wrapped with `withAuth()`. Key routes: `/api/spec-files/*` (DELETE supports `?prefix=X` for server-side bulk delete), `/api/spec-files/rules`, `/api/spec-files/split-swagger`, `/api/spec-files/reimport` (validate-before-wipe reimport), `/api/flow-files`, `/api/flow-chat`, `/api/generate-flow-ideas`, `/api/generate-flow`, `/api/debug-analyze` (AI step debugging), `/api/run-scenario`, `/api/proxy/*` (generic API proxy), `/api/active-tests`, `/api/test-runs`, `/api/users`, `/api/api-keys`, `/api/audit-log`, `/api/projects`, `/api/project-members`, `/api/ai-credits`, `/api/project-variables`, `/api/version-auth/credential`, `/api/api-rules`, `/api/connections`
