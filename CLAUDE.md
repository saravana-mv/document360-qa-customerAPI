...
25+ Azure Functions. All wrapped with `withAuth()`. Key routes: `/api/spec-files/*` (DELETE supports `?prefix=X` for server-side bulk delete), `/api/spec-files/rules`, `/api/spec-files/split-swagger`, `/api/spec-files/reimport` (validate-before-wipe reimport), `/api/spec-files/enhance-example` (AI-rewrite spec MD example from a captured Try-it call; qa_manager+), `/api/flow-files`, `/api/flow-chat`, `/api/generate-flow-ideas`, `/api/generate-flow`, `/api/debug-analyze` (AI step debugging), `/api/run-scenario`, `/api/proxy/*` (generic API proxy), `/api/active-tests`, `/api/test-runs`, `/api/users`, `/api/api-keys`, `/api/audit-log`, `/api/projects`, `/api/project-members`, `/api/ai-credits`, `/api/project-variables`, `/api/version-auth/credential`, `/api/api-rules`, `/api/connections`

### Auth Flow
Entra ID SSO → `EntraGate` auto-login → `ProjectGate` redirects to `/projects` if no project selected → `withAuth()` extracts OID/project from claims → `withProjectRole()` enforces per-project membership → generic proxy at `/api/proxy/*` injects auth based on connection provider type (reads connection from `X-FF-Connection-Id`, which includes base URL, API version, and credentials) → browser never holds real API credentials

### Connections (Generic Auth)
Connections are managed centrally in Settings → Connections (`ConnectionsPage`, `ConnectionFormModal`). Each connection stores auth credentials, `baseUrl`, and `apiVersion` — making it the single source of truth for endpoint configuration. Supported provider types: `"oauth2"` | `"bearer"` | `"apikey_header"` | `"apikey_query"` | `"basic"` | `"cookie"`. Credentials are stored server-side (never returned to browser; `sanitize()` strips secrets, returns `hasSecret`/`hasCredential` booleans). Connections are stored in the `connections` Cosmos container. `connections.store.ts` manages CRUD and OAuth status polling (only OAuth connections are polled). **Draft connections**: Auto-detected connections from OpenAPI security schemes are created with `draft: true` and include detected `baseUrl`/`apiVersion` from the spec, which skips credential validation (credentials must be filled in later via Settings → Connections).

### Connect Endpoint Modal
`ConnectEndpointModal` (`src/components/explorer/ConnectEndpointModal.tsx`) is a Connection picker dropdown (shows all connection types with status indicators). Base URL and API Version are read from the selected connection (configured in Settings → Connections), not entered in the modal. Label is auto-derived from the selected connection's name. Generic proxy (`/api/proxy/*`) injects auth based on the connection's provider type (looks up connection doc from Cosmos via `X-FF-Connection-Id` header). `RunControls` reads `baseUrl`/`apiVersion` from the connection doc (source of truth) with fallback to versionConfig. Legacy per-user credential path (Azure Table Storage) preserved as fallback when no connectionId.
...
Eleven post-processors run in order after AI generates flow XML: (1) `fixMismatchedEndpointRefs` → (2) `injectCrossStepCaptures` → (3) `injectMissingRequiredFields` → (4) `injectSpecRequiredFields` → (5) `stripExtraRequestFields` → (6) `injectEndpointRefs` → (7) `injectRulesRequiredFields` → (8) `validateCaptures` → (9) `fixTimestampAssertions` → (10) `fixCircularAssertions` → (11) `fixBareAssertionFields`. All live in `api/src/lib/specRequiredFields.ts` except `injectMissingRequiredFields`, `fixMismatchedEndpointRefs`, `fixTimestampAssertions`, `fixCircularAssertions`, and `fixBareAssertionFields` (local in `generateFlow.ts`).

**Method filters**: #3/#4/#5/#7 only process POST/PUT/PATCH bodies. #1/#2/#6/#8/#9/#10/#11 process all methods. When adding a new post-processor or changing method filters, verify all write methods are covered.

**Resource mismatch detection**: `fixMismatchedEndpointRefs` (#1) runs FIRST. Detects steps where the path resource (e.g., `/categories`) doesn't match the endpointRef resource (e.g., `articles/create-article.md`) — strips the wrong ref so downstream processors don't inject fields from the wrong spec. Prevents AI hallucination where prerequisite steps reference the only available spec instead of the correct (missing) one.
...
**Safety net**: `stripExtraRequestFields` (#5) runs AFTER injectors (#3/#4) and removes any field not in the spec's `allRequestFields`. It's the last line of defense against extra fields — must cover POST (not just PATCH/PUT) since bulk action endpoints use POST with strict schemas.
...
All AI functions (`generateFlow`, `generateFlowIdeas`, `flowChat`, `editFlow`, `debugAnalyze`, `enhanceDocsExample`) share a single context loader: `api/src/lib/aiContext.ts`. `loadAiContext()` returns an `AiContext` object that centralizes loading of spec context, API rules, project variables, and entity dependencies. Helpers: `enrichSystemPrompt()` appends rules/vars/deps to system prompts; `formatUserContext()` builds the user-facing context block. Also exports `findMatchingSpec()` (moved from `debugAnalyze.ts`). When adding a new AI endpoint, use `loadAiContext()` instead of independently loading context pieces.
