...
Ten post-processors run in order after AI generates flow XML: (1) `fixMismatchedEndpointRefs` → (2) `injectCrossStepCaptures` → (3) `injectMissingRequiredFields` → (4) `injectSpecRequiredFields` → (5) `stripExtraRequestFields` → (6) `injectEndpointRefs` → (7) `injectRulesRequiredFields` → (8) `validateCaptures` → (9) `fixTimestampAssertions` → (10) `fixCircularAssertions`. All live in `api/src/lib/specRequiredFields.ts` except `injectMissingRequiredFields`, `fixMismatchedEndpointRefs`, `fixTimestampAssertions`, and `fixCircularAssertions` (local in `generateFlow.ts`).

**Method filters**: #3/#4/#5/#7 only process POST/PUT/PATCH bodies. #1/#2/#6/#8/#9/#10 process all methods. When adding a new post-processor or changing method filters, verify all write methods are covered.
